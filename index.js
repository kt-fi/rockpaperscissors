...
document.querySelector(".gameBox").style.height = 0;

let res;
const CPUResult = ["paperRes", "scissorsRes", "rockRes"]
let playerScore = 0;
let svg;
let player = {
    playerRes: null,
...
changeGameScreen = () =>{
    document.querySelector(".gameArea").style.visibility = "hidden";
    document.querySelector(".gameArea").style.height = 0;
    document.querySelector(".gameBox").style.visibility = "visible";
    document.querySelector(".gameBox").style.height = "100%";
}

const onSelection = () => {
    document.querySelector(".playerResult").classList.toggle("iconPlr")
    document.querySelector(".playerResult").classList.toggle(`${playerChoice}Res`)
    let newsvg = document.querySelector(".playerResult")
...
eventListeners = () => {  
    ...
    document.querySelector(".gameArea").style.visibility = "visible";
    document.querySelector(".gameArea").style.height = "100%";
    document.querySelector(".gameBox").style.visibility = "hidden";
    document.querySelector(".gameBox").style.height = 0;

    CPUResult.forEach(result => {
        document.querySelector(".playerResult").classList.remove(result)
    })

    CPUResult.forEach(result => {
        document.querySelector(".compResult").classList.remove(result)
    })
    })
}
eventListeners();